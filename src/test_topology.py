    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if args.save_results:               
                ...
                pred_classes = np.zeros((h0, w0), dtype=np.int64)

                thin_np = thin_pred.numpy()
                large_np = large_pred.numpy()

                pred_classes[thin_np == 1] = 127
                pred_classes[large_np == 2] = 255

                pred_np = (bin_pred.numpy() * 255).astype(np.uint8)
                gt_np   = bin_gt.numpy().astype(np.uint8)
                ...
                cv2.imwrite(os.path.join(save_results_dir, f"{base}.png"), pred_classes)
